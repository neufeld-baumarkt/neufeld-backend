filename="reklamationen.js">
// routes/reklamationen.js – VERSION MIT LFD_NR-UNTERSTÜTZUNG FÜR DIE LISTE
...
query =         SELECT            r.*,           COUNT(p.id) AS position_count,           MIN(p.lfd_nr) AS min_lfd_nr         FROM reklamationen r         LEFT JOIN reklamation_positionen p ON r.id = p.reklamation_id         GROUP BY r.id         ORDER BY r.datum DESC      ;
} else {
query =         SELECT            r.*,           COUNT(p.id) AS position_count,           MIN(p.lfd_nr) AS min_lfd_nr         FROM reklamationen r         LEFT JOIN reklamation_positionen p ON r.id = p.reklamation_id         WHERE r.filiale = $1         GROUP BY r.id         ORDER BY r.datum DESC      ;
params = [filiale];
}
...
// Der Rest der Datei bleibt EXAKT wie bei dir – keine Änderung!
...
const reklaQuery =       INSERT INTO reklamationen (         datum, letzte_aenderung, art, rekla_nr, lieferant, filiale, status,         ls_nummer_grund, versand, tracking_id       ) VALUES (         $1, CURRENT_DATE, $2, $3, $4, $5, $6, $7, $8, $9       ) RETURNING id;    ;
const reklaValues = [
data.datum || null,
data.art || null,
data.rekla_nr || null,
...
const posQuery =         INSERT INTO reklamation_positionen (           reklamation_id, artikelnummer, ean, bestell_menge, bestell_einheit,           rekla_menge, rekla_einheit         ) VALUES ($1, $2, $3, $4, $5, $6, $7)      ;
...
try {
await client.query('BEGIN');
const updateQuery =       UPDATE reklamationen SET         datum = $1,         letzte_aenderung = CURRENT_DATE,         art = $2,         rekla_nr = $3,         lieferant = $4,         filiale = $5,         status = $6,         ls_nummer_grund = $7,         versand = $8,         tracking_id = $9       WHERE id = $10    ;
const updateValues = [
data.datum || null,
data.art || null,
data.rekla_nr || null,
