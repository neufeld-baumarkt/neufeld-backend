...
// Fingerprint auf jeder Response (Build + Startzeit)
const BUILD_TAG = process.env.BUILD_TAG || 'prod';
const START_TS = new Date().toISOString();
app.use((req, res, next) => {
  res.setHeader('X-Build-Tag', BUILD_TAG);
  res.setHeader('X-Started-At', START_TS);
  next();
});
...
app.get('/api/health', (req, res) => {
  res.json({ ok: true, buildTag: BUILD_TAG, startedAt: START_TS });
});
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const userRes = await pool.query(
      'SELECT id, name, password, role, filiale FROM public.users WHERE name = $1 LIMIT 1',
      [name]
    );
    ...
    const token = jwt.sign(
      { id: user.id, role: user.role, name: user.name, filiale: user.filiale || null },
    ...
    return res.json({
      token,
      user: { id: user.id, name: user.name, role: user.role, filiale: user.filiale || null },
    });
  } catch (err) {
 ...
 * - nur Filiale (Owner)
 * - PIN check über core.filiale_pins (filiale_id + display_name)
 * - Lockout: 5 fails → 15 min
 * - Idempotent: wenn Task schon executed/admin_closed/canceled → 200 ohne Änderung
 */
app.post('/api/tasks/:id/execute', verifyToken(), async (req, res) => {
  ...
  try {
    ...
    // 0) Task laden (Owner-check + Status)
    ...
    // Idempotent: wenn schon final -> 200, ohne PIN-Check, ohne Event
    ...
    if (pRes.rows.length === 0) {
      // Event pin_failed (ohne preiszugeben was falsch war)
      await pool.query(
        `INSERT INTO core.task_events (task_id, event_type, actor_user_id, meta)
    ...
    if (!ok) {
      ...
      const lock = nextFails >= PIN_MAX_FAILS
    ...
    // 2) PIN ok → reset Lockout + Task ausführen (Transaction)
    await pool.query('BEGIN');
    ...
    if (updRes.rows.length === 0) {
      // Task wurde zwischenzeitlich final → idempotent zurückgeben
      await pool.query('ROLLBACK');
 ...
 * Body: { note: "..." }
 * - nur Zentrale-Rollen (Admin, Supervisor, Geschäftsführer, Manager-1)
 * - note Pflicht
 * - Wirkung: status=admin_closed + admin_closed_* Felder + Event 'admin_closed'
 * - Idempotent: wenn schon admin_closed → 200 ohne neues Event
 */
app.post('/api/tasks/:id/admin-close', verifyToken(), async (req, res) => {
  ...
  try {
    ...
    // Idempotent: schon admin_closed → 200 ohne Event
    ...
    if (updRes.rows.length === 0) {
      // Race: Task wurde zwischenzeitlich admin_closed → idempotent zurückgeben
      await pool.query('ROLLBACK');
